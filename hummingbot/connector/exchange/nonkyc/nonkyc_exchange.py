...
class NonKYCExchange(ExchangeBase):
    ...
    def restore_tracking_states(self, saved_states: Dict[str, any]):
        ...
        self._in_flight_orders.update({
            key: NonKYCInFlightOrder.from_json(value)
    ...
    async def _format_trading_rules(self, symbols_info: Dict[str, Any]) -> Dict[str, TradingRule]:
        ...
        for rule in symbols_info:
            try:
                trading_pair = await NonKYCAPIOrderBookDataSource.trading_pair_associated_to_exchange_symbol(rule["symbol"])
    ...
    async def _api_request(self,
        ...
        headers: dict = {}
        if is_auth_required:
            headers: dict = self._nonkyc_auth.get_headers(method, f"{Constants.REST_URL}/{endpoint}",
                                                          params)
        # Build request coro
            ...
            for response, tracked_order in zip(responses, tracked_orders):
                ...
                if isinstance(response, NonKYCAPIError):
                    ...
                    if err.get('code') == 20002:
                        ...
                        continue
                elif "userProvidedId" not in response:
                    self.logger().info(f"_update_order_status userProvidedId not in resp: {response}")
                    continue
                else:
    ...
    def _process_order_message(self, order_msg: Dict[str, Any]):
        ...
        """
        client_order_id = order_msg["userProvidedId"]
        ...
        # update order
        tracked_order.executed_amount_base = Decimal(order_msg["executedQuantity"])
        tracked_order.executed_amount_quote = Decimal(order_msg["price"]) * Decimal(order_msg["executedQuantity"])
    ...
    def _process_balance_message(self, balance_update):
        ...
        for account in balance_update:
            ...
            self._account_balances[asset_name] = Decimal(str(account["held"])) + Decimal(str(account["available"]))
        ...
        async for event_message in self._iter_user_event_queue():
            try:
                ...
                if method == Constants.WS_METHODS["USER_TRADES"]:
                    ...
                    for order_msg in params or []:
                        self._process_order_message(order_msg)
                elif isinstance(account_balances, list) and "asset" in account_balances[0]:
    ...
    async def get_open_orders(self) -> List[OpenOrder]:
        ...
        ret_val = []
        for order in result:
            if Constants.HBOT_BROKER_ID not in order["userProvidedId"]:
                continue
            ...
            ret_val.append(
                OpenOrder(
                    ...
                    executed_amount=Decimal(str(order["executedQuantity"])),
                    status=order["status"],
